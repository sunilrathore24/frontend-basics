# Torque — Architecture Overview (10-Minute Interview Guide)

## What is Torque?

Torque is an **enterprise-grade Angular UI platform** built by Saba Software (Cornerstone OnDemand).
...
│
├── libs/                                  # ── Libraries (the core of Torque) ─
│   │
│   ├── core/                              # @torque/core
...
│   │
│   ├── ui/                                # @torque/ui — 60+ UI Components
│   │   ├── accordion/                     #   Each folder = secondary entry point
│   │   ├── alert/                         #   e.g. import from '@torque/ui/alert'
│   │   ├── badge/
│   │   ├── breadcrumb/
│   │   ├── button/
│   │   ├── button-group/
...
│   │   └── ... (50+ more)
│   │
│   ├── forms/                             # @torque/forms — 20+ Form Components
│   │   ├── auto-complete/
│   │   ├── checkbox/
│   │   ├── datetimepicker/
...
│   │   └── ...
│   │
│   ├── framework/                         # @torque/framework — App Shell
│   │   └── src/lib/
...
│   │
│   ├── patterns/                          # @torque/patterns — Design Patterns
│   │   ├── color-palette/
│   │   ├── typography/
│   │   └── iconography/
│   │
│   ├── assets/                            # @torque/assets — Global Styles
│   │   └── src/lib/smacss/scss/
...
│   │       └── torque.scss                #   └─ Master stylesheet entry
│   │
...
│   │
│   ├── lego/                              # @torque/lego — Angular ↔ WC Bridge
│   │   ├── icon/
│   │   ├── shared/
│   │   └── sandbox/
│   │
...
import { TrqButtonModule } from '@torque/ui';
```

`tsconfig.base.json` maps **80+ path aliases** (`@torque/ui/datagrid`, `@torque/forms/auto-complete`, etc.) to their source entry points. This is critical for enterprise apps where bundle size = load time.

### 2. NgRx + Facade Pattern for Framework State

The application shell (`@torque/framework`) manages navigation, layout, and settings via NgRx:
...
Selectors (30+ selectors for nav items, layout flags, branding)
    ↓
TrqFrameworkFacade ← Components inject THIS, not Store directly
...
Both `@torque/core` and `@torque/framework` expose composable provider functions:
...
│  (@torque/ui, /forms)      (@lego/elements-galaxy)│
│  ┌──────────────┐          ┌──────────────┐       │
│  │ TrqButton    │          │ galaxy-header│       │
│  │ TrqDatagrid  │          │ galaxy-menu  │       │
│  │ TrqModal     │          │ galaxy-search│       │
│  └──────────────┘          └──────────────┘       │
│        ↑                          ↑               │
│   Angular CDK              Lego CDK (base styles) │
│  (@torque/core/cdk)        (@lego/cdk)            │
└─────────────────────────────────────────────────┘
```
...
Per-component:
├── _button-config.scss    → Structural variables ($trq-btn-padding, etc.)
...
`@torque/core/platform` provides enterprise infrastructure that's independent of UI:
...
CI Pipeline (TeamCity/Jenkins):
  nx affected:lint    → ESLint (TS) + @torque/eslint-plugin (templates)
